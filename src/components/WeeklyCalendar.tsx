...
const WeeklyCalendar: React.FC = () => {
  ...
  const [modalContent, setModalContent] = useState<
    "createRoom" | "makeReservation" | null
  >(null);
  ...
  const makeReservation = async (roomId: string, date: string) => {
    try {
      ...
      const response = await fetch(
        "http://localhost:1234/api/rooms/reservations",
        {
          method: "POST",
          headers: {
  ...
  const cancelReservation = async (roomId: string, date: string) => {
    try {
      console.log("Tentative d'annulation de réservation:", { roomId, date });
      const token = localStorage.getItem("token");
      if (!token) {
        throw new Error("Token d'authentification non trouvé");
      }
      ...
      const response = await fetch(
        `http://localhost:1234/api/rooms/${roomId}/reservations`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      console.error(
        "Erreur détaillée lors de l'annulation de la réservation:",
        error
      );
      setError(
  ...
  const handleDeleteRoom = async () => {
    if (selectedRoom) {
      try {
        const response = await fetch(
          `http://localhost:1234/api/rooms/${selectedRoom._id}`,
          {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleAddRoom = async (room: { name: string; equipment: string }) => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.message || "Erreur lors de la création de la salle"
        );
      }
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-lg">
      ...
      <div className="grid grid-cols-8 gap-px bg-gray-200 rounded-lg overflow-hidden">
        <div className="font-bold text-center bg-[#25a8a6] text-white p-2">
          Salles
        </div>
        {days.map((day) => (
          <div
            key={day.toString()}
            className="font-bold text-center bg-[#25a8a6] text-white p-2"
          >
        ...
        {modalContent === "makeReservation" && selectedCell && (
          <MakeReservation
            ...
            room={{
              ...
              nom:
                rooms.find((room) => room._id === selectedCell.roomId)?.name ||
                "",
            }}
            onClose={() => setIsModalOpen(false)}
            onReserve={() =>
              makeReservation(selectedCell.roomId, selectedCell.date)
            }
            onCancelReservation={() =>
              cancelReservation(selectedCell.roomId, selectedCell.date)
            }
          />
        )}
      </Modal>
